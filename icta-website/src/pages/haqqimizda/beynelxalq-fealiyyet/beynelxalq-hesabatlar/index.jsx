...
const BeynelxalqHesabatlar = () => {
  ...
  useEffect(() => {
    ...
    if (result) {
      setData({
        ...
        spectrTitle: "Beynəlxalq hesabatlar",
      });
    }
  }, [getSectionData]);
  ...
  const repo_resource = [
    { name: data.reponame1, kb: data.repokb1 },
    { name: data.reponame2, kb: data.repokb2 },
    { name: data.reponame3, kb: data.repokb3 },
    { name: data.reponame4, kb: data.repokb4 },
    { name: data.reponame5, kb: data.repokb5 },
  ].filter((item) => item.name && item.kb); // Filter out empty entries
  ...
  return (
    <div>
      ...
      {repo_resource.map((el, index) => (
        <div key={index} className="pt-5">
          <div className="flex justify-between gap-3 items-center bg-[rgb(243,246,250)] p-5">
            <div>
              <h1 className="text-[18px] font-medium pt-2 mb-1">{el.name}</h1>
              <p className="text-[rgb(152,162,179)] mb-3">{el.kb}</p>
            </div>
            <div className="flex gap-5">
              <IoEyeSharp className="text-[20px] text-[rgb(152,162,179)] cursor-pointer" />
              <RiFileDownloadFill className="text-[20px] text-[rgb(152,162,179)] cursor-pointer" />
            </div>
          </div>
        </div>
      ))}
...
export default BeynelxalqHesabatlar;
